...
import { gettinhthanh } from '../../Models/getdata';
import Backbutton from'../Backbutton'

function Cart() {
    let navigate = useNavigate()

    const { isLoading, error, data, isFetching, refetch } = useQuery(['tinhthanh'], gettinhthanh, {})
    ...
    const handleClickmomo = async()=>{
        const response = await axios.post('/api/user/momo', { product: product, fullname: user.fullname, email: user.gmail, date: new Date(), phone_number: user.phone_number, address: address + ", " + ward + ", " + district + ", " + city  });

        window.location.replace(response.data.link)
    }
    let handlesubmit = async (e) => {
        e.preventDefault();
        let result = await axios.post('/api/user/checkout', { fullname: user.fullname, email: user.gmail, date: new Date(), phone_number: user.phone_number, address: address + ", " + ward + ", " + district + ", " + city })
        ...
        <>
            <div class="container">
                ...
                <div class="row ">
                    ...
                    <form class="needs-validation form-billing bg-slate-700 p-4 rounded-xl shadow text-white xl:ml-10 md:ml-10 sm:ml-0" onSubmit={handlesubmit}>
