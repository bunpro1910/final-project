...
import { deleteinvoice } from '../../Models/getdata'
function Invoice() {
  ...
  let getinvoice = () => axios.get(`/api/manager/invoice`, { params: { s_id: search.s_id, s_userid: search.s_userid, s_customer: search.s_customer, s_email: search.s_email, s_phonenumber: search.s_phonenumber, s_cartid: search.s_cartid, s_status: search.s_status } }).then((res) => res.data)
  ...
  const handleChangeStatus = (inv, status) => async (e) => {
    let result = await axios.post('/api/manager/changestatus', { id: inv.id, status: status })
  ...
  const handledelete = async (e) => {
    let result = await deleteinvoice(invoice) 
    if (result.data.isSuccess) {
      toast.success(result.data.message)
      setshow(false)
    } else {
  ...
  return (
    <>
          ...
          <DialogContentText tabIndex={-1} >
            <form >
              ...
              <div class="mb-3">
                ...
                <select class="custom-select d-block w-100 form-control" id="state" required onChange={(e) => { search.s_status = e.target.value; setsearch({ ...search }) }} value={search.s_status}>
      ...
      <div className="w-[70%] mr-auto ml-auto">
          <button className=' bg-slate-600 rounded-xl p-2 hover:bg-slate-700 !text-white' onClick={handleClickOpenSearch} >Search</button>
        </div>
